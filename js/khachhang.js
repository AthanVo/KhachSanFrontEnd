...
async function loadAvailableRooms() {
    const token = localStorage.getItem('jwtToken');
    try {
        const response = await fetch('http://localhost:5000/api/BookingOnlineAPI/GetRoomsOnline', {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`
...
async function bookRoom() {
    ...
    try {
        const response = await fetch('http://localhost:5000/api/BookingOnlineAPI/BookRoomOnline', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`
...
async function processPaymentOnline(datPhongId) {
    ...
    try {
        ...
        const response = await fetch('http://localhost:5000/api/BookingOnlineAPI/ProcessPaymentOnline', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`
